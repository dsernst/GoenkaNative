...
function Calendar(props: Props) {
  const [{ month, selected }, setThisState] = useState<State>({ month: dayjs(), selected: dayjs() })
  ...
  const sitsByDate = cachedGroupBy(props.history, sit => dayjs(sit.date).format('YYYY-MM-DD'))
  ...
  return (
    <View style={{ marginHorizontal: 25 }}>
      ...
      >
        <TouchableOpacity
          onPress={() => setThisState({ month: month.subtract(1, 'month'), selected: null })}
        ...
        >
          <Text style={{ color: '#fffb', fontSize: 16, textAlign: 'center' }}>{month.format('MMMM YYYY')}</Text>
        </TouchableOpacity>
        <TouchableOpacity
          disabled={inCurrMonth}
          onPress={() => setThisState({ month: month.add(1, 'month'), selected: null })}
          style={{
            borderColor: 'red',
            borderWidth: 0,
            ...
            if (counter === 1 && position < Number(firstDayOfMonth.format('d'))) {
              ...
              return (
                <TouchableOpacity
                  activeOpacity={0.5}
                  key={key}
                  onPress={() => setThisState({ month, selected: !isSelected ? day : null })}
