...
function BarView(props: Props) {
  ...
  const numDaysToShow = Math.ceil(
    endOfToday.diff(_.last(history)!.date, 'day', true),
  );
  ...
  const sitsByHalfDay = cachedGroupBy(history, sit =>
    dayjs(sit.date).format('YYYY-MM-DDa'),
  );
  ...
  const maxElapsed = _.reduce(
    sitsByHalfDay,
    (memo, sits) => Math.max(memo, _.sum(_.map(sits, 'elapsed'))),
    0,
  );
  const yLabels = [
    ..._.range(180, 14, -15).filter(l => l < maxElapsed + 15),
    '',
  ];
        ...
        renderItem={({index, item: range}) => (
          <View key={index}>
            <TouchableOpacity
              activeOpacity={0.8}
              onPress={() =>
                setSelected(selected === range ? undefined : range)
              }
              style={{
                justifyContent: 'flex-end',
              ...
              {sitsByHalfDay[range]?.filter(sit => sit.elapsed).length ? (
                ...
                <EmptyBarRange
                  {...{barWidth, eveningPurple, morningYellow, range, selected}}
                />
              )}
            </TouchableOpacity>
      ...
      {selected && (
        <View style={{flexDirection: 'row', marginTop: 10}}>
          <View style={{paddingLeft: 5, paddingRight: 20, paddingTop: 2}}>
            <Text style={{color: '#fff7'}}>
              {dayjs(selected).format('ddd MMM D')}
            </Text>
