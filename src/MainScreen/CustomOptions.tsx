...
export default (props: Props) => {
  const {customDuration, hasChanting, hasExtendedMetta, setState, toggle} =
    props;
  ...
  return (
    <View style={{paddingHorizontal: 24, paddingTop: 5}}>
      ...
      <Picker
        ...
        onValueChange={(newDuration: number) =>
          setState({customDuration: newDuration})
        }
        ...
        {[1, 3, ..._.range(5, 61, 5), ..._.range(75, 121, 15)].map(
          (num: number) => (
            <Picker.Item
              key={num}
              label={((n: number) => {
