  ...
  const unsubscribeFromAuth = auth().onAuthStateChanged(user => {
      ...
      .onSnapshot(results => {
        ...
        setState({
          onlineSits: results.docs
            ...
            .map(
              (doc): {date: FirebaseFirestoreTypes.Timestamp} & OnlineSit => ({
                id: doc.id,
                ...doc.data(),
              }),
            )
    ...
    unsubscribeFromOutgoingFriendRequests = firestore()
      ...
      .onSnapshot(results => {
        if (!results) {
          return console.log(
            '🚫 db snapshot: no results for outgoingFriendRequests',
          );
        }
    ...
    unsubscribeFromIncomingFriendRequests = firestore()
      ...
      .onSnapshot(results => {
        if (!results) {
          return console.log(
            '🚫 db snapshot: no results for incomingFriendRequests',
          );
        }
    ...
    unsubscribeFromRecentlyJoinedContacts = firestore()
      ...
      .onSnapshot(async results => {
        if (!results) {
          return console.log(
            '🚫 db snapshot: no results for recentlyJoinedContacts',
          );
        }
