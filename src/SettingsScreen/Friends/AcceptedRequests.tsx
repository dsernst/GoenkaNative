...
function AcceptedRequests({
  ...
  return (
    <>
      ...
      {allRequests.map(({ name, phone, request, toggle_notifs, wants_notifs }) => (
        <View key={request.id} style={{ flexDirection: 'row', marginTop: 15, opacity: wants_notifs ? 1 : 0.3 }}>
          <TouchableOpacity
            onPress={() =>
              Alert.alert(
                `Unfriend ${name}?`,
          ...
          <View style={{ marginLeft: 5 }}>
            ...
            <Text style={{ color: '#fff5' }}>&nbsp; {prettyDisplayPhone(phone)}</Text>
          </View>
          <Switch
            onValueChange={() =>
              firestore()
