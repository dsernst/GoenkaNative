...
function DailyNotificationSettings(props: Props) {
  ...
  const TimePickersTuples: [
    ...
    [
      ...
      'morning',
      amPickerVisible,
      moment('2020-01-01 11:59 AM', 'YYYY-MM-DD hh:mm A').toDate(), // maxDate
    ...
    [
      ...
      pmPickerVisible,
      undefined, // no maxDate
      moment('2020-01-01 12:00 PM', 'YYYY-MM-DD hh:mm A').toDate(), // minDate
      setPmPickerVisible,
      newTime => ({pmNotificationTime: newTime}),
    ],
  ];

  console.log('AdjustTimeTuples: ', AdjustTimeTuples);
  return (
    <>
      ...
      {TimeSwitchesTuples.map(([key, color, isOn, Icon, iconName]) => (
        <TouchableOpacity
          ...
          <View style={{flexDirection: 'row'}}>
            ...
            />
            <Text style={{color: '#fffa', fontSize: 16}}>
      ...
      {TimePickersTuples.map(
        ([
          ...
          <DateTimePickerModal
            date={time}
            customHeaderIOS={() => <Text>{`Set ${key} time`}</Text>}
            isVisible={isVisible}
            key={key}
            maximumDate={maxDate}
            minimumDate={minDate}
